...
def fetch_remote_template(
    url, folder='.', branch='master', tempdir=tempfile.mkdtemp()
):
    ...
    # TODO: include any folder? or look `manifest.yaml` for templates metadata?
    template_folders = [
    ...
    # TODO: couldn't figure out the library-native function
    template_repo.git.checkout(branch, folder)

    return os.path.join(tempdir, folder)
...
def validate_template(template_path):
    ...
    for folder in required_folders:
        if not os.path.isdir(os.path.join(template_path, folder)):
            raise ValueError(
                f'Folder {folder} is required for the template to be valid'
            )
    for file in required_files:
        if not os.path.isfile(os.path.join(template_path, file)):
            raise ValueError(
...
def create_from_template(
    ...
    with client.commit():
        client.import_from_template(Path(template_path), metadata, force)
